...
public class BasketAlignCommand extends CommandBase {
    ...
    public BasketAlignCommand(
            MecanumDriveSubsystem mecanumDrive,
            BasketSensorSubsystem basketSensor,
    ...
    public BasketAlignCommand(
            ...
            BasketSensorSubsystem basketSensorSubsystem,
            PinpointSubsystem pinpoint) {
    ...
    public void execute() {
        ...
        Pose2d p =
                dtController.calculate(
                        AutoConstants
                                .scorePos, // .plus(new Transform2d(new Translation2d(pose.getX(),
                        // pose.getY()), new Rotation2d())),
                        pinpoint.getPose(), // new Pose2d(x, y, new Rotation2d()),
                        // new Pose2d(basketSensor.getSensorRight(), basketSensor.getSensorLeft(),
                        // new Rotation2d()),
                        pinpoint.getVelocity());
        double dtFactor =
                1.0; // Math.pow(Math.cos(targetHeading - pinpoint.getPose().getHeading()), 3.0);
    ...
    public void end(boolean interrupted) {
        if (!interrupted && !interruptSupplier.getAsBoolean()) {
            if (Math.abs(
                            pinpoint.getPose().getRotation().getDegrees()
                                    - AutoConstants.scorePos.getRotation().getDegrees())
                    < 2.5) {
                Pose2d newPose =
                        new Pose2d(
                                AutoConstants.scorePos.getX()
                                        - basketSensor.getSensorRight()
                                        + xOffset,
                                AutoConstants.scorePos.getY()
                                        - basketSensor.getSensorLeft()
                                        + yOffset,
                                new Rotation2d());
