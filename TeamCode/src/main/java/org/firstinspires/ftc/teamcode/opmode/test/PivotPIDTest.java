...
public class PivotPIDTest extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (!isStopRequested()) {
            ...
            CommandScheduler.getInstance().schedule(pivot.openloopC(() -> PivotConstants.slowDropPower * extension.getCurrentInches()/SlideConstants.maxExtension));
            extension.setManualControl(true);
            CommandScheduler.getInstance().schedule(extension.openloopC(0.0));
            wrist.setWrist(IntakeConstants.slightOpenPos);
