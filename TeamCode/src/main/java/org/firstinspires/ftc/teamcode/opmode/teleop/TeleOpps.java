...
//        );
        ...
        new Trigger(() -> driverPad.getTrigger(GamepadKeys.Trigger.RIGHT_TRIGGER)>=0.8).whileActiveOnce(
                subPos()
        ).whenInactive(new IntakeRetractCommand(wrist, pivot, extension, turret).alongWith(
                new IntakeControlCommand(intake, IntakeConstants.closedPos, 0)
            )
        );


        new Trigger(() -> driverPad.getTrigger(GamepadKeys.Trigger.LEFT_TRIGGER)>=0.8).whileActiveOnce(
