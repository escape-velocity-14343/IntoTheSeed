...
public class IntakeRetractCommand extends SequentialCommandGroup {
    ...
    public IntakeRetractCommand(WristSubsystem wrist, PivotSubsystem pivot, ExtensionSubsystem extend, TurretSubsystem turret) {
        ElapsedTime pivotTimer = new ElapsedTime();
        addCommands(
                new InstantCommand(pivotTimer::reset),
                new InterruptCommand(new PivotCommand(pivot, PivotConstants.intakeReadyPos),
                        () -> pivot.getPivotVelocity() < AutoConstants.autoscoreMaxPivotVel && pivotTimer.seconds() > 0.2),
                ...
                new WristCommand(wrist, IntakeConstants.foldedPos).whenFinished(() -> Log.i("5", "Intake Retract command"))
